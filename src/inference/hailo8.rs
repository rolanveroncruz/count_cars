...
use super::{CountCarsIntelligence, DetectedLicensePlate, TrackedObject};
...
pub struct Hailo8Manager {
    ...
    pub plate_network_group: hailo_configured_network_group,
    pub plate_input_vstream: hailo_input_vstream,
    pub plate_input_size: usize,
    pub plate_output_vstreams: Vec<hailo_output_vstream>,
    pub plate_output_sizes: Vec<usize>,
    pub plate_is_nms: bool,
    // OCR/LPRNet pipeline (configured at init, reused per-frame)
    pub ocr_network_group: hailo_configured_network_group,
    pub ocr_input_vstream: hailo_input_vstream,
    pub ocr_input_size: usize,
    pub ocr_output_vstreams: Vec<hailo_output_vstream>,
    pub ocr_output_sizes: Vec<usize>,
}
...
impl Hailo8Manager {
    ...
    fn configure_hef_on_vdevice(
        the_hef: hailo_hef,
        vdevice: hailo_vdevice,
        network_group: &mut hailo_configured_network_group,
        name: &str,
    ) {
        unsafe {
            let mut vehicle_configure_params: hailo_configure_params_t = std::mem::zeroed();
            if hailo_init_configure_params_by_vdevice(
                the_hef,
                vdevice,
    ...
    fn get_all_vstream_infos(
        the_hef: hailo_hef,
        all_vstream_infos: &mut [hailo_vstream_info_t; HAILO_MAX_STREAMS_COUNT as usize],
        num_all_vstreams: &mut usize,
    ) {
        unsafe {
    ...
    fn create_input_vstream(
        network_group: hailo_configured_network_group,
        input_vstream: &mut hailo_input_vstream,
    ) {
        unsafe {
            let mut input_params: [hailo_input_vstream_params_by_name_t;
                HAILO_MAX_STREAMS_COUNT as usize] = std::mem::zeroed();
    ...
    fn create_output_vstream(
        ...
        unsafe {
            let mut output_params: [hailo_output_vstream_params_by_name_t;
                HAILO_MAX_STREAMS_COUNT as usize] = std::mem::zeroed();
    ...
    pub fn new(vehicle_path: &str, plate_path: &str, ocr_path: &str) -> Self {
        ...
        unsafe {
            ...
            Self::configure_hef_on_vdevice(
                vehicle_hef,
                vdevice,
                &mut vehicle_network_group,
                "vehicle",
            );

            // 5. Query all vstream infos from the HEF (HailoRT 4.23.0 API)
            let mut all_vstream_infos: [hailo_vstream_info_t; HAILO_MAX_STREAMS_COUNT as usize] =
                std::mem::zeroed();
            ...
            Self::separate_vstream_infos(
                &mut all_vstream_infos,
                num_all_vstreams,
                &mut h2d_infos,
                &mut d2h_infos,
            );
            println!(
                "Vehicle HEF: {} input(s), {} output(s)",
                h2d_infos.len(),
                d2h_infos.len()
            );
            ...
            vehicle_output_vstreams =
                Self::create_output_vstream(vehicle_network_group, &d2h_infos);

            // 9. Create output vstreams
            // 10. Compute input size and output sizes for buffer allocation
            let mut expected_in_size: usize = 0;
            if hailo_get_input_vstream_frame_size(vehicle_input_vstream, &mut expected_in_size)
                != HAILO_SUCCESS
            {
            ...
            for i in 0..d2h_infos.len() {
                ...
                match order {
                    HAILO_FORMAT_ORDER_HAILO_NMS => {
                        println!("Vehicle Output {} Format:LEGACY_NMS", i);
                    }
                    HAILO_FORMAT_ORDER_HAILO_NMS_BY_CLASS => {
                        println!("Vehicle Output {} Format: NMS_BY_CLASS", i)
                    }
                    HAILO_FORMAT_ORDER_HAILO_NMS_BY_SCORE => {
                        println!("Vehicle Output {} Format: NMS_BY_SCORE", i)
                    }
                    _ => println!(
                        "Vehicle Output {} Format: RAW TENSOR (Enum ID: {})",
                        i, order
                    ),
                }
                ...
                if hailo_get_output_vstream_frame_size(
                    vehicle_output_vstreams[i],
                    &mut expected_out_size,
                ) != HAILO_SUCCESS
                {
            ...
            println!(
                "Vehicle input size: {},  NMS output: {}",
                vehicle_input_size, vehicle_is_nms
            );
            // ========================================================
            // ✅ CONFIGURE PLATE YOLO PIPELINE
            // ========================================================

            println!("Step 5: Configure the Plate HEF on the VDevice");
            ...
            let mut expected_in_size:usize = 0;
            if hailo_get_input_vstream_frame_size(plate_input_vstream, & mut expected_in_size,)!=HAILO_SUCCESS{
                panic!("Failed to get input vstream frame size");
            }
            plate_input_size = expected_in_size;
            // Compute pl ae output sizes
            for i in 0..d2h_infos.len(){
                let mut expected_out_size:usize = 0;
                if hailo_get_output_vstream_frame_size(
                    plate_output_vstreams[i],
                    &mut expected_out_size,
                )!=HAILO_SUCCESS{
                    panic!("Failed to get output vstream frame size");
                }
                plate_output_sizes.push(expected_out_size);
            }

            // ========================================================
            // ✅ CONFIGURE OCR LPRNET PIPELINE
            // ========================================================
            println!("Step 5: Configure the OCR HEF on the VDevice");
            ...
            let mut expected_in_size:usize = 0;
            if hailo_get_input_vstream_frame_size(
                ocr_input_vstream,
                &mut expected_in_size
            )!=HAILO_SUCCESS {
                panic!("Failed to get OCR input vstream frame size");
            }
            ocr_input_size = expected_in_size;

            for i in 0..d2h_infos.len(){
                let mut expected_out_size:usize = 0;
                if hailo_get_output_vstream_frame_size(
                    ocr_output_vstreams[i],
                    &mut expected_out_size
                )!=HAILO_SUCCESS {
                    panic!("Failed to get OCR output vstream frame size");
                }
                ocr_output_sizes.push(expected_out_size);
            }
            println!("OCR input size: {}, output size(s): {:?}", ocr_input_size, ocr_output_sizes);
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            if !self.vehicle_input_vstream.is_null() {
                hailo_release_input_vstreams(&mut self.vehicle_input_vstream, 1);
            }
            ...
            if !self.ocr_hef.is_null() {
                hailo_release_hef(self.ocr_hef);
            }
            if !self.plate_hef.is_null() {
                hailo_release_hef(self.plate_hef);
            }
            if !self.vehicle_hef.is_null() {
                hailo_release_hef(self.vehicle_hef);
            }
            if !self.vdevice.is_null() {
                hailo_release_vdevice(self.vdevice);
            }
        }
    }
}
...
impl CountCarsIntelligence for Hailo8Manager {
        ...
        for (i, output_vs) in self.vehicle_output_vstreams.iter().enumerate() {
            ...
            unsafe {
                ...
                let output_floats = unsafe {
                    std::slice::from_raw_parts(output_buf.as_ptr() as *const f32, float_count)
                };
    ...
    fn locate_plate(&mut self, cropped_vehicle: &Mat) -> Option<DetectedLicensePlate> {
        let detections = self.detect_plate(cropped_vehicle);
        detections
            .into_iter()
            .max_by(|a,b| a.score.total_cmp(&b.score))
            .map(|plate| {
                let scale_x = cropped_vehicle.cols() as f32 / 416.0;
                let scale_y = cropped_vehicle.rows() as f32 / 416.0;
                DetectedLicensePlate{
                    confidence: plate.score,
                    x: (plate.xmin * scale_x) as i32,
                    y: (plate.ymin * scale_y) as i32,
                    width:  ((plate.xmax - plate.xmin) * scale_x) as i32,
                    height: ((plate.ymax - plate.ymin) * scale_y) as i32,
                }
            })
    }

    fn recognize_text(&mut self, cropped_plate: &Mat) -> Option<String> {
        self.recognize_text(cropped_plate)
    }
}
...
fn decode_lprnet_ctc(buffer: &[u8]) -> Option<String> {
    ...
    let char_map = [
        ...
        '-', // The final index is usually the CTC Blank character
    ];
    ...
    let output_floats =
        unsafe { std::slice::from_raw_parts(buffer.as_ptr() as *const f32, float_count) };
...
fn parse_nms_output(buffer: &[u8], orig_w: f32, orig_h: f32, objects: &mut Vec<TrackedObject>) {
    ...
    for class_id in 0..num_classes {
        ...
        let class_name = COCO_NAMES.get(class_id).unwrap_or(&"unknown").to_string();
